...
import { ControlUnitDetailScene } from "~/components/store/control-units/details/product-detail.scene"
import { getRegion } from "@lib/data/regions"
import { getControlUnitContent } from "~/lib/data/control-unit-content"
import { HttpTypes } from "@medusajs/types"

export const revalidate = 0

export default async function ControlUnitDetailsPage(props: { params: Promise<{ id: string }> }) {
  ...
  // Fetch control unit content from Payload CMS
  const controlUnitContent = await getControlUnitContent(product.id)

  return (
    <ControlUnitDetailScene
      product={product}
      relatedProducts={relatedProducts}
      countryCode={countryCode}
      controlUnitContent={controlUnitContent}
    />
  )
}
