...
/**
 * Raw ice bath content data from Payload (before transformation)
 */
interface IceBathContentRaw {
  id: string
...
/**
 * Transformed ice bath content data (ready for frontend consumption)
 */
export interface IceBathContentData {
  id: string
  medusaProductId: string
  productName: string
  advantages: string[]
...
/**
 * Fetch ice bath content from Payload CMS by Medusa product ID
 *
 * @param medusaProductId - The Medusa product ID (e.g., prod_01HXXX...)
 * @returns IceBathContentData or null if not found
 */
export async function getIceBathContent(
  medusaProductId: string
): Promise<IceBathContentData | null> {
  try {
    ...
    const raw = result.docs[0] as unknown as IceBathContentRaw
    ...
    console.error(
      `Error fetching ice bath content for product ${medusaProductId}:`,
      error
    )
    return null
  }
