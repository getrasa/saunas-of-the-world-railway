...
/**
 * Raw accessory content data from Payload (before transformation)
 */
interface AccessoryContentRaw {
  id: string
...
/**
 * Transformed accessory content data (ready for frontend consumption)
 */
export interface AccessoryContentData {
  id: string
  medusaProductId: string
  productName: string
  advantages: string[]
...
/**
 * Fetch accessory content from Payload CMS by Medusa product ID
 *
 * @param medusaProductId - The Medusa product ID (e.g., prod_01HXXX...)
 * @returns AccessoryContentData or null if not found
 */
export async function getAccessoryContent(
  medusaProductId: string
): Promise<AccessoryContentData | null> {
  try {
    ...
    const raw = result.docs[0] as unknown as AccessoryContentRaw
    ...
    console.error(
      `Error fetching accessory content for product ${medusaProductId}:`,
      error
    )
    return null
  }
