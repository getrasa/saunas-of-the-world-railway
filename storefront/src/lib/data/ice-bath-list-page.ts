...
export interface IceBathListHeroData {
  title: string
  subtitle: string
  description: string
...
export interface IceBathListPageData {
...
interface IceBathListPageGlobalRaw {
  hero?: HeroRaw
}

export async function getIceBathListPageData(): Promise<IceBathListPageData> {
  try {
    ...
    const page = await payload.findGlobal({
      ...
      depth: 2,
    }) as IceBathListPageGlobalRaw

    // Transform hero
    let hero: IceBathListHeroData | null = null
    if (page?.hero) {
    ...
    console.error('Error fetching ice bath list page data:', error)
    return {
      hero: null,
    }
  }
