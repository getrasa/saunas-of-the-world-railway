...
export interface AccessoryListHeroData {
  title: string
  subtitle: string
  description: string
...
export interface AccessoryListPageData {
  hero: AccessoryListHeroData | null
}
...
interface AccessoryListPageGlobalRaw {
  hero?: HeroRaw
}

export async function getAccessoryListPageData(): Promise<AccessoryListPageData> {
  try {
    ...
    const page = await payload.findGlobal({
      ...
      slug: 'accessory-list-page',
      depth: 2,
    }) as AccessoryListPageGlobalRaw

    // Transform hero
    let hero: AccessoryListHeroData | null = null
    if (page?.hero) {
    ...
    console.error('Error fetching accessory list page data:', error)
    return {
      hero: null,
    }
  }
