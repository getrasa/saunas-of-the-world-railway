...
import { ControlUnitDetailGallery } from "./product-detail-gallery"
import { ControlUnitDetailAccordion } from "./product-detail-accordion"
import { ControlUnitDetailInfo } from "./product-detail-info"
import { useCart } from "~/contexts/cart-context"
import { addToCart } from "@lib/data/cart"
import type { ControlUnitContentData } from "@lib/data/control-unit-content"

interface ControlUnitDetailSceneProps {
  product: HttpTypes.StoreProduct
  relatedProducts: HttpTypes.StoreProduct[]
  countryCode: string
  controlUnitContent: ControlUnitContentData | null
}
...
export function ControlUnitDetailScene({
  product,
  relatedProducts,
  countryCode,
  controlUnitContent,
}: ControlUnitDetailSceneProps) {
  ...
  const advantages = useMemo(() => {
    if (controlUnitContent?.advantages && controlUnitContent.advantages.length > 0) {
      return controlUnitContent.advantages
    }
  ...
  }, [controlUnitContent, product.metadata])

  // Extract specifications - prioritize Payload content
  const specifications = useMemo(() => {
    if (controlUnitContent?.specifications && Object.keys(controlUnitContent.specifications).length > 0) {
      return controlUnitContent.specifications
    }
    const meta = product.metadata as any
  ...
  const downloads = useMemo(() => {
    return controlUnitContent?.downloads || []
  }, [controlUnitContent])

  // Breadcrumb items
  const breadcrumbItems = [
    { label: "Products", href: "/shop" },
    { label: "Controllers", href: "/shop/control-units" },
  ...
  return (
    <>
      <ProductDetailLayout>
        ...
        <BackButton href="/shop/control-units" className="mb-8" />

        <ProductDetailLayout.Grid>
          <ProductDetailLayout.LeftColumn>
            <ControlUnitDetailGallery
              images={images}
              productName={product.title || "Product"}
            />

            <ControlUnitDetailAccordion
      ...
      {relatedProductCards.length > 0 && (
        <ProductRecommendations
          ...
          linkBuilder={(id) => `/shop/control-units/${id}`}
          className="mt-16"
        />
      )}
    </>
