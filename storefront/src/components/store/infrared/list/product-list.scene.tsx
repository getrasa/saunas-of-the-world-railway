...
import type { InfraredListHeroData } from "~/lib/data/infrared-list-page"

interface InfraredProductListProps {
  products: ProductCardData[]
  countryCode: string
  hero: InfraredListHeroData | null
}

export function InfraredListScene({ products, countryCode, hero }: InfraredProductListProps) {
        ...
        <div className="mx-auto max-w-[1512px]">
          ...
          >
            {products.map((product) => (
              <Link key={product.id} href={`/shop/infrared/${product.id}`}>
                <ProductCard
                  product={product}
