...
import { InfraredDetailGallery } from "./product-detail-gallery"
import { InfraredDetailAccordion } from "./product-detail-accordion"
import { InfraredDetailInfo } from "./product-detail-info"
import { useCart } from "~/contexts/cart-context"
import { addToCart } from "@lib/data/cart"
import type { InfraredContentData } from "@lib/data/infrared-content"

interface InfraredDetailSceneProps {
  product: HttpTypes.StoreProduct
  relatedProducts: HttpTypes.StoreProduct[]
  countryCode: string
  infraredContent: InfraredContentData | null
}
...
export function InfraredDetailScene({
  product,
  relatedProducts,
  countryCode,
  infraredContent,
}: InfraredDetailSceneProps) {
  ...
  const advantages = useMemo(() => {
    if (infraredContent?.advantages && infraredContent.advantages.length > 0) {
      return infraredContent.advantages
    }
  ...
  }, [infraredContent, product.metadata])

  // Extract specifications - prioritize Payload content
  const specifications = useMemo(() => {
    if (infraredContent?.specifications && Object.keys(infraredContent.specifications).length > 0) {
      return infraredContent.specifications
    }
    const meta = product.metadata as any
  ...
  const downloads = useMemo(() => {
    return infraredContent?.downloads || []
  }, [infraredContent])

  // Breadcrumb items
  const breadcrumbItems = [
    { label: "Products", href: "/shop" },
    { label: "Infrared Saunas", href: "/shop/infrared" },
  ...
  return (
    <>
      <ProductDetailLayout>
        ...
        <BackButton href="/shop/infrared" className="mb-8" />

        <ProductDetailLayout.Grid>
          <ProductDetailLayout.LeftColumn>
            <InfraredDetailGallery
              images={images}
              productName={product.title || "Product"}
            />

            <InfraredDetailAccordion
      ...
      {relatedProductCards.length > 0 && (
        <ProductRecommendations
          ...
          linkBuilder={(id) => `/shop/infrared/${id}`}
          className="mt-16"
        />
      )}
    </>
