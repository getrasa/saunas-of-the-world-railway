...
import { IceBathDetailGallery } from "./product-detail-gallery"
import { IceBathDetailAccordion } from "./product-detail-accordion"
import { IceBathDetailInfo } from "./product-detail-info"
import { useCart } from "~/contexts/cart-context"
import { addToCart } from "@lib/data/cart"
import type { IceBathContentData } from "@lib/data/ice-bath-content"

interface IceBathDetailSceneProps {
  product: HttpTypes.StoreProduct
  relatedProducts: HttpTypes.StoreProduct[]
  countryCode: string
  iceBathContent: IceBathContentData | null
}
...
export function IceBathDetailScene({
  product,
  relatedProducts,
  countryCode,
  iceBathContent,
}: IceBathDetailSceneProps) {
  ...
  const advantages = useMemo(() => {
    if (iceBathContent?.advantages && iceBathContent.advantages.length > 0) {
      return iceBathContent.advantages
    }
  ...
  }, [iceBathContent, product.metadata])

  // Extract specifications - prioritize Payload content
  const specifications = useMemo(() => {
    if (iceBathContent?.specifications && Object.keys(iceBathContent.specifications).length > 0) {
      return iceBathContent.specifications
    }
    const meta = product.metadata as any
  ...
  const downloads = useMemo(() => {
    return iceBathContent?.downloads || []
  }, [iceBathContent])

  // Breadcrumb items
  const breadcrumbItems = [
    { label: "Products", href: "/shop" },
    { label: "Ice Baths", href: "/shop/ice-baths" },
  ...
  return (
    <>
      <ProductDetailLayout>
        ...
        <BackButton href="/shop/ice-baths" className="mb-8" />

        <ProductDetailLayout.Grid>
          <ProductDetailLayout.LeftColumn>
            <IceBathDetailGallery
              images={images}
              productName={product.title || "Product"}
            />

            <IceBathDetailAccordion
      ...
      {relatedProductCards.length > 0 && (
        <ProductRecommendations
          ...
          linkBuilder={(id) => `/shop/ice-baths/${id}`}
          className="mt-16"
        />
      )}
    </>
