...
import type { AccessoryListHeroData } from "~/lib/data/accessory-list-page"

interface AccessoryProductListProps {
  products: ProductCardData[]
  countryCode: string
  hero: AccessoryListHeroData | null
}

export function AccessoryListScene({ products, countryCode, hero }: AccessoryProductListProps) {
        ...
        <div className="mx-auto max-w-[1512px]">
          ...
          >
            {products.map((product) => (
              <Link key={product.id} href={`/shop/accessories/${product.id}`}>
                <ProductCard
                  product={product}
